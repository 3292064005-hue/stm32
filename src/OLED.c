...
/***************************************************************************************
  ...
  * 
  * 程序名称：				0.96寸OLED显示屏驱动程序（STM32F103硬件I2C接口）
  * 程序创建时间：			2023.10.24
  * 当前程序版本：			V2.0
  * 当前版本发布时间：		2024.10.20
  * 
...
#define OLED_AFIO_CLK_ENABLE()      __HAL_RCC_AFIO_CLK_ENABLE()
#define OLED_I2C_ADDR               OLED_I2C_ADDR_8BIT
...
static I2C_HandleTypeDef hOLED_I2C;

/**
  * 函    数：OLED硬件I2C初始化
  * 参    数：无
  * 返 回 值：无
  * 说    明：使用STM32F103的I2C1外设，PB6为SCL，PB7为SDA
  */
static void OLED_I2C_Init(void)
{
	...
	/*使能AFIO、GPIOB和I2C1时钟*/
	OLED_AFIO_CLK_ENABLE();
	OLED_I2C_GPIO_CLK_ENABLE();
	OLED_I2C_CLK_ENABLE();
	
	/*PB6/PB7配置为复用开漏输出，对应I2C1默认引脚*/
	...
	/*初始化I2C1，400kHz快速模式；如连线较长或不稳定，可改为100000*/
	hOLED_I2C.Instance = OLED_I2C_INSTANCE;
	hOLED_I2C.Init.ClockSpeed = 400000;
	hOLED_I2C.Init.DutyCycle = I2C_DUTYCYCLE_2;
	hOLED_I2C.Init.OwnAddress1 = 0;
	hOLED_I2C.Init.AddressingMode = I2C_ADDRESSINGMODE_7BIT;
	hOLED_I2C.Init.DualAddressMode = I2C_DUALADDRESS_DISABLE;
	hOLED_I2C.Init.OwnAddress2 = 0;
	hOLED_I2C.Init.GeneralCallMode = I2C_GENERALCALL_DISABLE;
	hOLED_I2C.Init.NoStretchMode = I2C_NOSTRETCH_DISABLE;
	
	HAL_I2C_DeInit(&hOLED_I2C);
	HAL_I2C_Init(&hOLED_I2C);
}
...
/**
  * 函    数：OLED通过硬件I2C发送数据块
  * 参    数：Control 控制字节，0x00表示命令，0x40表示数据
  * 参    数：Data    要发送数据的起始地址
  * 参    数：Count   要发送的数据数量
  * 返 回 值：无
  */
static void OLED_I2C_Send(uint8_t Control, const uint8_t *Data, uint16_t Count)
{
	uint8_t Buffer[129];
	...
	HAL_I2C_Master_Transmit(&hOLED_I2C, OLED_I2C_ADDR, Buffer, Count + 1, OLED_I2C_TIMEOUT);
}
...
/**
  * 函    数：OLED设置反色显示
  * 参    数：IsInverse 0正常显示，非0反色显示
  * 返 回 值：无
  */
void OLED_SetInverse(uint8_t IsInverse)
{
	OLED_WriteCommand(IsInverse ? 0xA7 : 0xA6);
}
...
{
	uint8_t CharIndex;
	uint8_t AsciiChar = (uint8_t)Char;
	...
	if (FontSize == OLED_8X16)		//字体为宽8像素，高16像素
	{
		...
		OLED_ShowImage(X, Y, 8, 16, OLED_F8x16[CharIndex]);
	}
	else	//默认字体为宽6像素，高8像素
	{
		/*将ASCII字模库OLED_F6x8的指定数据以6*8的图像格式显示*/
		OLED_ShowImage(X, Y, 6, 8, OLED_F6x8[CharIndex]);
	}
}
...
void OLED_ShowString(int16_t X, int16_t Y, const char *String, uint8_t FontSize)
{
	...
	while (String[i] != '\0')	//遍历字符串
	{
		...
		{
			Glyph = OLED_FindChineseGlyph(SingleChar);
			if (FontSize == OLED_8X16)		//给定字体为8*16点阵
			{
				/*将字模库OLED_CF16x16的指定数据以16*16的图像格式显示*/
				OLED_ShowImage(X + XOffset, Y, 16, 16, Glyph->Data);
				XOffset += 16;
			}
			else	//给定字体为6*8点阵
			{
...
void OLED_ShowFloatNum(int16_t X, int16_t Y, double Number, uint8_t IntLength, uint8_t FraLength, uint8_t FontSize)
{
	...
	FraNum = (uint32_t)round(Number * PowNum);	//将小数乘到整数，同时四舍五入，避免显示误差
